...
public class PlayfieldPanel extends JPanel implements PlayfieldDisplay, Player{
	...
	public PlayfieldPanel(Game pGame, Console pConsole){
		super();
	...
	private void saveCoordinates(int x, int y) {
		...
		case FIRST:
			...
			if(!playfield.isOccupied(x, y)){
				...
				if(m.isInvalid() || !GameLogic.testMove(m,playfield) || (jumpIsPossible() && m.getMoveType() == MoveType.STEP)){
					//TODO cancel move
				...
				if(m.getMoveType() == MoveType.JUMP){
					//multiJumpTesting
					list = GameLogic.testForMultiJump(coords[0][0],coords[0][1], playfield);
					list.toFirst();
